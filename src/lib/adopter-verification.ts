...
const ADOPTER_VERIFICATIONS_STORAGE_KEY = "petpal_adopter_verifications"
...
/**
 * Get all adopter verification records
 */
export async function getAdopterVerifications(): Promise<AdopterVerification[]> {
  try {
    const stored = await AsyncStorage.getItem(ADOPTER_VERIFICATIONS_STORAGE_KEY)
    if (stored) {
      return JSON.parse(stored)
    } else {
      await AsyncStorage.setItem(ADOPTER_VERIFICATIONS_STORAGE_KEY, JSON.stringify(defaultVerifications))
...
/**
 * Save adopter verification records
 */
export async function saveAdopterVerifications(verifications: AdopterVerification[]): Promise<void> {
  try {
    await AsyncStorage.setItem(ADOPTER_VERIFICATIONS_STORAGE_KEY, JSON.stringify(verifications))
  } catch (error) {
    console.error("Error saving adopter verifications:", error)
  }
}

/**
 * Update verification status and add review information
 */
export async function updateVerificationStatus(
  ...
  reviewedBy: string,
  notes?: string,
): Promise<AdopterVerification | null> {
  try {
    const verifications = await getAdopterVerifications()
    ...
    if (index !== -1) {
      ...
      await saveAdopterVerifications(verifications)
 ...
 */
export async function getVerificationById(id: string): Promise<AdopterVerification | undefined> {
  try {
    const verifications = await getAdopterVerifications()
...
/**
 * Initialize adopter verification data
 */
export async function initializeAdopterVerifications(): Promise<void> {
  try {
    const stored = await AsyncStorage.getItem(ADOPTER_VERIFICATIONS_STORAGE_KEY)
    if (!stored) {
      await AsyncStorage.setItem(ADOPTER_VERIFICATIONS_STORAGE_KEY, JSON.stringify(defaultVerifications))
      console.log("Initialized default adopter verification data")
    }
  } catch (error) {
    console.error("Error initializing adopter verifications:", error)
  }
}
