...
// Match the type from app/(tabs)/adopter/applications/page.tsx
...
export default function ApplicationDetailsScreen() {
  ...
  // Get ID from multiple sources to ensure it works in different navigation contexts
  ...
  // If ID is still undefined, try getting it from React Navigation route params
  if (!id) {
    try {
      // @ts-ignore - Access route.params safely
      const routeParams = navigation.getState()?.routes?.find(r => r.name === 'ApplicationDetails')?.params;
      if (routeParams?.id) {
        id = String(routeParams.id);
      }
    } catch (error) {
      console.warn("Error getting React Navigation params:", error);
    }
  }
  
  // Keep router as a fallback, but don't rely on it for primary navigation
  ...
  useEffect(() => {
    const loadApplicationData = async () => {
      try {
        ...
        if (app && (app.adopterId === user?.id || app.adopterId === 'demo-user')) {
          ...
          // Create an ApplicationWithPet object
          const applicationWithPet: ApplicationWithPet = {
            ...app,
          ...
          setApplication(applicationWithPet);
          if (petData) {
            setPet(petData);
          }
          ...
          console.error('Application not found or access denied');
        }
      } catch (error) {
    ...
    // Only load data if we have both a user and an ID
    if (user && id) {
      loadApplicationData();
    } else if (!id) {
      setLoading(false); // Don't keep showing loading if no ID
    }
  }, [id, user]);
      ...
      <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              // Use goBack() for safer navigation or navigate to a known screen
              try {
                navigation.goBack();
              } catch (error) {
                // Fallback to ApplicationList which seems to exist based on AdopterDashboardScreen
        ...
        <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.outline }]}>
          <View style={styles.petInfoContainer}>
            ...
            <View style={styles.petInfo}>
              <Text style={[styles.petName, { color: theme.colors.onSurface }]}>
                {application.petName || 'Pet'}
              </Text>
              <Text style={[styles.petBreed, { color: theme.colors.outline }]}>
                {application.petBreed || 'Unknown Breed'}
              </Text>
            </View>
          ...
          <View style={styles.petMetaInfo}>
            ...
            <View style={styles.metaItem}>
              ...
              <Text style={[styles.metaText, { color: theme.colors.onSurface }]}>
                {application.shelterName || 'Shelter'}
              </Text>
            </View>
          </View>
        </View>
        ...
        >
          ...
          <View style={styles.contactButtons}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // Use React Navigation consistently
                navigation.navigate('Chat', { petId: application.petId });
              }}
            >
